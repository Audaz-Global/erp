  ...
  try {
    ...
    const agent = await prisma.agent.create({
      data: {
        ...
        code: data.code || null,
        aliases: data.aliases ? JSON.stringify(Array.isArray(data.aliases) ? data.aliases : String(data.aliases).split(',').map((value: string) => value.trim()).filter(Boolean)) : null,
        equipmentScopes: data.equipmentScopes ? JSON.stringify(data.equipmentScopes) : null,
  ...
  try {
    ...
    const agent = await prisma.agent.update({
      ...
      data: {
        ...
        aliases: data.aliases !== undefined ? JSON.stringify(Array.isArray(data.aliases) ? data.aliases : String(data.aliases).split(',').map((value: string) => value.trim()).filter(Boolean)) : undefined,
        equipmentScopes: data.equipmentScopes !== undefined ? JSON.stringify(data.equipmentScopes) : undefined,
