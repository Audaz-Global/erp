...
export const extractClientData = async (text: string, contextRules: string = '', mediaParts: any[] = [], modalFocus?: 'AIR' | 'SEA') => {
  ...
  try {
          ...
          properties: {
            ...
            route: {
              ...
              properties: {
                ...
                connections: { type: 'string', description: 'Conexões do voo (ex: via MIA) ou escalas de porto (ex: transbordo em Algeciras), APENAS quando explicitamente mencionadas no texto. NUNCA infira ou deduza uma conexão/rota que não esteja escrita na solicitação, mesmo que pareça uma rota plausível ou comum entre a origem e o destino — não use conhecimento geral sobre a rede de rotas de companhias aéreas ou armadores. Retorne string vazia se for direto ou se a rota/escalas não forem mencionadas.' },
    ...
    - **Conexões do Voo ou do Porto (CRÍTICO — não inventar)**: Identifique se há menção EXPLÍCITA a escalas, aeroportos de conexão intermediários (ex: via MIA, via FRA) ou portos de escala/transbordo (ex: transbordo em Algeciras). Salve essa informação em "connections" SOMENTE quando estiver escrita no texto. NUNCA infira, deduza ou complete uma conexão/rota plausível a partir de conhecimento geral sobre companhias aéreas ou armadores (ex: não assuma que um trajeto costuma passar por um hub conhecido) — se a solicitação não menciona escalas, o embarque é presumido direto e "connections" deve ser string vazia "".
    ...
    - **Peso de produto ≠ peso do volume/embarque (CRÍTICO)**: um peso rotulado como "product weight"/"item weight" ou pareado com "product dimensions" (dimensão de UM produto) é peso de catálogo/cotação comercial de UMA peça — NÃO inclui embalagem, engradado, palete, shrink wrap etc. NUNCA multiplique esse peso unitário pela quantidade de peças/produtos vendidos (ex: "Qty: 135 units" × "Weight: 31.55lbs") para preencher gross_weight_kg quando o texto indicar que essas peças estão fisicamente consolidadas em menos volumes do que sua quantidade individual (ex: "shrink wrapped as one package"). Nesse cenário, se não houver nenhum peso declarado no nível do volume/embarque (packing list, peso total consolidado, invoice de frete), retorne gross_weight_kg vazio/0 — não invente um total, deixe para conferência manual do operador. Peso por volume explícito (ex: "Box 1: ... 12 kg", "crate weight: 50kg") continua podendo ser somado normalmente, pois aí é peso do volume, não do produto.
    ...
    - **packages_count**: O número de volumes/caixas físicas do embarque (ex: 3 wooden boxes), NÃO a quantidade de peças/produtos individuais dentro dos volumes. Termos como "pcs", "pieces", "units", "qty", "eaches" (ou "Qty: 135 units" numa cotação comercial de venda) referem-se à quantidade de PRODUTO vendido, não à quantidade de embalagens físicas — nunca use esse número como packages_count sem confirmar que cada peça vai numa embalagem própria e separada.
    - **Glossário de embalagens (volume = 1 unidade física de carga)**: caixa de papelão/carton/ctn/box, engradado de madeira/wooden crate, palete/pallet/skid, tambor/drum, saco/bag, fardo/bale, pacote encolhível/shrink-wrapped bundle. Quando o texto contar EMBALAGENS usando esses termos (ex: "3 wooden crates", "12 cartons", "2 pallets", "1 shrink-wrapped bundle"), esse número é o packages_count.
    - **Frase de consolidação sempre vence (CRÍTICO)**: se o texto descrever explicitamente como a carga está fisicamente embalada em conjunto — "packed as one", "shrink wrapped as one package", "consolidated on one pallet", "packed in X cartons/crates/pallets", "no pallet" seguido de menção a um único pacote — use esse número como packages_count, MESMO que ele contradiga ou seja muito menor que a quantidade de produtos/peças mencionada em outro lugar do e-mail (ex: cotação comercial informando "Qty: 135 units" mas o e-mail de embarque dizendo "shrink wrapped as one package" = packages_count 1, não 135).
    - **Dimensão de produto ≠ dimensão de embalagem (CRÍTICO)**: dimensões rotuladas como "product dimensions", "item dimensions" ou dimensões de UM produto/peça NUNCA devem ser multiplicadas pela quantidade de produtos para simular várias caixas/volumes — isso infla artificialmente o peso taxável. Se o texto afirma consolidação em menos volumes (ex: "1 package") mas não informa a dimensão real da embalagem consolidada, NÃO calcule nem invente essa dimensão combinando quantidade de produto com dimensão de produto — omita o item de "dimensions" nesse caso e deixe para conferência manual do operador (veja também a regra de Peso de produto ≠ peso do volume acima, que se aplica ao mesmo cenário).
    - **Dimensões com Quantidade (CRÍTICO)**: Cada item no array "dimensions" DEVE obrigatoriamente iniciar com a quantidade de volumes correspondente àquela dimensão usando o formato "[Quantidade]x [Comprimento]x[Widht]x[Altura] cm" (ex: "1x 50x50x28 cm", "2x 50x50x13 cm", "3x 37.5*31*37 cm" e "9x 63x41.5x38 cm"). Se a quantidade não for colocada na frente de cada dimensão, a cubagem acumulada falhará. Identifique também termos como "ctns", "ctn", "carton" ou "cartons" no packing list ou e-mail, pois eles significam caixas de papelão e devem ser contados como volumes físicos.
              ...
              properties: {
                ...
                rate_options: {
                  ...
                  items: {
                    type: 'object',
                    properties: {
                      label: { type: 'string', description: 'Nome curto da opção que reflita a diferença REAL entre elas — como o agente descreveu (ex: "Direto", "Via Miami", "1 escala", "Standard", "Express") ou, se o agente só numerou as opções sem nomeá-las mas a diferença de verdade é o dia/data de saída, use isso como rótulo (ex: "Saída Segunda-feira", "Saída 15/10"). Só use "Opção 1"/"Opção 2" genérico como último recurso, quando não há nenhuma outra diferença identificável a nomear.' },
                      freight_value: { type: 'number' },
                      freight_currency: { type: 'string' },
                      transit_time: { type: 'string' },
                      carrier: { type: 'string' },
                      frequency: { type: 'string', description: 'Dia/data de saída OU frequência desta opção específica, quando o agente informar (ex: "Saída toda segunda-feira", "Embarque em 15/10", "2x por semana"). Preencha mesmo que o valor pareça igual entre as opções — se for a única diferença real entre elas (ex: mesma rota/prazo/companhia, mudando só o dia de saída), é essa a informação que o usuário precisa pra escolher.' }
                    },
                    required: ['label', 'freight_value']
                  },
                  description: 'Preencha SOMENTE quando o agente oferecer explicitamente DUAS OU MAIS alternativas de frete/prazo para o MESMO embarque na mesma resposta (ex: rota direta vs com escala, standard vs express, via porto/aeroporto A vs B, ou mesma rota com dias de saída diferentes). NÃO é uma alternativa: equipamentos diferentes (20 pés vs 40 pés), destinos diferentes, ou embarques distintos — isso não entra aqui. Deixe o array vazio quando houver apenas uma opção de frete na resposta.'
                },
